...
function getTable($table_name) {
	...
	if ($driver == 'sqlite') {
		...
		if ($driver == 'mysql') {
			$results = getResult("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '$database' AND TABLE_NAME = '$table_name'");
		} elseif ($driver == 'pgsql') {
			$results = getResult("SELECT COLUMN_NAME AS \"COLUMN_NAME\", DATA_TYPE AS \"DATA_TYPE\", CHARACTER_MAXIMUM_LENGTH AS \"CHARACTER_MAXIMUM_LENGTH\", IS_NULLABLE AS \"IS_NULLABLE\", COLUMN_DEFAULT AS \"COLUMN_DEFAULT\" FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_CATALOG = '$database' AND TABLE_NAME = '$table_name'");
		} else {
			$results = getResult("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_CATALOG = '$database' AND TABLE_NAME = '$table_name'");
		}
